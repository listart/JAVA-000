...
CREATE TABLE IF NOT EXISTS `ec`.`sku` (
  ...
  `name` VARCHAR(120) NOT NULL DEFAULT '',
  `attr_names` JSON NOT NULL DEFAULT '{}',
  `attr_values` JSON NOT NULL DEFAULT '{}',
  `price` DECIMAL(10,2) NOT NULL DEFAULT 0.00,
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_sku_item1_idx` (`item_id` ASC) VISIBLE,
  CONSTRAINT `fk_sku_item1`
    FOREIGN KEY (`item_id`)
    REFERENCES `ec`.`item` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `ec`.`sub_order` (
  ...
  `pay_fee` DECIMAL(10,2) NOT NULL DEFAULT 0.00,
  PRIMARY KEY (`id`),
  INDEX `fk_sub_order_order1_idx` (`order_id` ASC) VISIBLE,
  INDEX `fk_sub_order_sku1_idx` (`sku_id` ASC) VISIBLE,
  CONSTRAINT `fk_sub_order_order1`
    FOREIGN KEY (`order_id`)
    REFERENCES `ec`.`order` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_sub_order_sku1`
    FOREIGN KEY (`sku_id`)
    REFERENCES `ec`.`sku` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `ec`.`order_detail` (
  ...
  `pay_fee` DECIMAL(10,2) NOT NULL DEFAULT 0.00,
  PRIMARY KEY (`id`),
  INDEX `fk_order_detail_sub_order1_idx` (`sub_order_id` ASC) VISIBLE,
  CONSTRAINT `fk_order_detail_sub_order1`
    FOREIGN KEY (`sub_order_id`)
    REFERENCES `ec`.`sub_order` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
